"""
Interpreter for the programming language OK
...
#O is the value of the current memory location, k is the current memory location
o = []
k  = 0
...
def lineread(line):
	...
	if line.startswith("Secret"):
		...
		return 0
	if line.startswith("\t"):
		print("Tabs are the crutches that exist to make bad programmers feel better")
		cool = False
		return 0
...
def runfile(file_list):	
	...
	while current_line < len(file_list):
		...
		if o[k] > 510:
			o[k] = o[k] % 510
		#print(k)
path = raw_input("Enter a file name or path\n")
	
file_master = grabfile(path)
runfile(file_master)
#printfile(file_master)